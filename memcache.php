...
$MEMCACHE_SERVERS[] = 'unix:///var/run/memcached.socket';
...
function sendMemcacheCommands($command){
	...
	foreach($MEMCACHE_SERVERS as $server){
		...
		//$host = $strs[0];
		//$port = $strs[1];
		$hostport = gethostport($server);
		$result[$server] = sendMemcacheCommand($hostport[0],$hostport[1],$command);
	}
	return $result;
}
...
list($host,$port) = gethostport($server);	
